describe "Collection" do
  ...
  it "supports a disjunction of filters" do
    filtered = collection.filter Criteria.artist('Sting') | Criteria.artist('Pearl Jam')
    filtered.map(&:name).should eq ["Fields of Gold", "Mad About You", "Brain of J.F.K", "Jeremy"]
  end

  it "supports negative criteria" do
    filtered = collection.filter !Criteria.artist('Sting')
    filtered.artists.should_not include 'Sting'
  end
